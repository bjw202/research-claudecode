...
- 폴더 내 파일: **숫자 프리픽스 + 관점/역할 기반 이름**
  - `00-`: 종합 보고서(synthesis). 독자가 가장 먼저 읽을 파일.
  - `01-`\~`89-`: 상세 보고서(detail). Researcher 산출물 등.
  - `90-`\~`99-`: 부속 자료. Critic review, process log 등.
  - 예: `00-synthesis.md`, `01-eis-methodology.md`, `99-critic-review.md`
...
- Critic: 논리적 허점, 반례, 과도한 일반화 탐지
  - **Researcher 2명 이상인 리서치에서 Critic은 필수**. 아래 5개 체크리스트를 수행한다:
    1. 도메인 적용성: 인용된 연구/데이터가 대상 도메인에 유효한가
    2. 수치 근거 검증: 핵심 수치의 출처와 조건이 명시되었는가
    3. 서브에이전트 간 상충점: Researcher 산출물 간 모순이 있는가
    4. 누락 관점: 비용, 실행 가능성, 실패 모드가 다뤄졌는가
    5. 확신도 교정: 근거 대비 과도한 확신이 있는가
...
모든 Researcher 서브에이전트 프롬프트에 아래 4가지 지침을 포함한다:
...
최종 통합 시 아래 섹션을 포함한다. 해당 없으면 "해당 없음, 이유: ..." 명시. 억지로 채우지 않는다.
